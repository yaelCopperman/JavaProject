/*
 * Yael Copperman/Kimchi
 * 207618257
 * yaelikimchi@gmail.com
 */
import primitives.*;
	import static java.lang.System.out;

public class Main {
	
	/**
	 * Test program for the 1st stage
	 *
	 * @author Dan Zilberstein
	 */
		/**
		 * Main program to tests initial functionality of the 1st stage
		 * 
		 * @param args irrelevant here
		 */
		public static void main(String[] args) {

			try { // test zero vector
				new Vector(0, 0, 0);
				out.println("ERROR: zero vector does not throw an exception");
			} catch (Exception e) {
			}

			Vector v1 = new Vector(1, 2, 3);
			Vector v2 = new Vector(-2, -4, -6);
			Vector v3 = new Vector(0, 3, -2);

			// test length..
			if ((v1.lengthSquared() - 14)!=0)
				out.println("ERROR: lengthSquared() wrong value");
			if ((new Vector(0, 3, 4).length() - 5)!=0)
				out.println("ERROR: length() wrong value");
			
			// test Dot-Product
			if ((v1.dotProduct(v3))!=0)
				out.println("ERROR: dotProduct() for orthogonal vectors is not zero");
			if ((v1.dotProduct(v2) + 28)!=0)
				out.println("ERROR: dotProduct() wrong value");

			// test Cross-Product
			try { // test zero vector
				v1.crossProduct(v2);
				out.println("ERROR: crossProduct() for parallel vectors does not throw an exception");
			} catch (Exception e) {
			}
			Vector vr = v1.crossProduct(v3);
			if ((vr.length() - (v1.length() * v3.length()))!=0)
				out.println("ERROR: crossProduct() wrong result length");
			if ((vr.dotProduct(v1))!=0 ||(vr.dotProduct(v3))!=0)
				out.println("ERROR: crossProduct() result is not orthogonal to its operands");

			// test vector normalization vs vector length and cross-product
			Vector v = new Vector(1, 2, 3);
			Vector vCopy = new Vector(v);
			Vector vCopyNormalize = vCopy.normalize();
			if (vCopy != vCopyNormalize)
				out.println("ERROR: normalize() function creates a new vector");
			if ((vCopyNormalize.length() - 1)!=0)
				out.println("ERROR: normalize() result is not a unit vector");
			Vector u = v.normalized();
			if (u == v)
				out.println("ERROR: normalizated() function does not create a new vector");
			
			out.println("If there were no any other outputs - all tests succeeded!");
		}
	}
